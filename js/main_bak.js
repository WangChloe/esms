...
var estVerify = {
	...
	EstSuccess: function() {
		debugger;
		var dataStr = {'orderBase.customerId':parseFloat($("#orderBase-Cid").val()),'orderBase.inc':$("#orderBase-inc").val(),
				'orderBase.typ':$("#orderBase-typ").val(),'orderBase.price':$("#orderBase-price").val(),
				'orderBase.notes':$("#orderBase-notes").val(),'orderBase.tTime':$("#orderBase-tTime").val(),'orderBase.sTime':$("#orderBase-sTime").val()};
		$.ajax({
			type: "GET",
				...
				if (!parseInt(data.code)) {
					...
					if (orderBase.length) {
						...
						for (var i = 0; i < orderBase.length; i++) {
							...
							switch (i % 3) {
								case 0:
									oImg.attr('src', 'img/property_1.jpg');
									break;
								case 1:
									oImg.attr('src', 'img/property_2.jpg');
									break;
								case 2:
									oImg.attr('src', 'img/property_3.jpg');
									break;
							}
